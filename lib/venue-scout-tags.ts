...
export function scoutAttributesFromForm(input: {
  ...
  if (capacity) attrs[SCOUT_ATTR_KEYS.capacity] = [capacity];
...
): { website: string; phone: string; capacity: string; tags: string[] } {
  return {
    website: attributes[SCOUT_ATTR_KEYS.website]?.[0] || "",
    phone: attributes[SCOUT_ATTR_KEYS.phone]?.[0] || "",
    capacity: attributes[SCOUT_ATTR_KEYS.capacity]?.[0] || "",
