...
import {
  ...
  applyForwardGeocodedLocation,
  applyReverseGeocodedLocation,
} from "@/lib/geocoding";
import {
...
import {
  ...
  resolveMapsUrlForLocation,
  searchGeocodeAddresses,
  type SpaceLocationApiMode,
} from "@/lib/space-location-geocode";
...
/** Paarl town centre — default when no coordinates are set. */
...
type SpaceLocationSectionProps = {
...
export function SpaceLocationSection({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        const next = await searchGeocodeAddresses(apiMode, query, 5, controller.signal);
        setSuggestions(next);
  ...
  const selectSuggestion = useCallback(
    (result: GeocodedAddress) => {
      ...
      onChange(applyForwardGeocodedLocation(valueRef.current, result, true));
      clearAddressSuggestionState();
      setGeoMessage("Address selected. Pin placed on map.");
    },
    [clearAddressSuggestionState, onChange]
  );

  async function geocodeSearchQuery(query: string, successMessage: string) {
    const matches = await searchGeocodeAddresses(apiMode, query, 1);
    if (matches.length === 0) {
      setGeoMessage(
    ...
    skipAutocompleteRef.current = true;
    onChange(applyForwardGeocodedLocation(valueRef.current, matches[0], true));
    clearAddressSuggestionState();
  ...
  async function applyGoogleMapsUrl(rawInput?: string) {
    ...
    try {
      const resolved = await resolveGoogleMapsUrlClient(raw, (url) =>
        resolveMapsUrlForLocation(apiMode, url)
      );
  ...
  const handleMapPinChange = useCallback(
    ...
    [clearAddressSuggestionState, onChange, readOnly, reverseGeocodePin]
  );
      ...
      {geoMessage ? (
        <p
          className={`mt-3 text-sm ${
            ...
            geoMessage.includes("could not be resolved") ||
