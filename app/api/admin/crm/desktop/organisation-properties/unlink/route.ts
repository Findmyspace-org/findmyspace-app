...
import { unlinkPropertyFromOrganisation } from "@/lib/crm-desktop/organisation-property-link";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await unlinkPropertyFromOrganisation(auth.adminClient, {
    ...
    console.error("[crm/organisation-properties/unlink]", error);
    return NextResponse.json(
      { error: "Failed to unlink property." },
      { status: 500 }
    );
  }
}
