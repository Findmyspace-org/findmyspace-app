...
export async function POST(
  ...
  if (!comment) {
    return NextResponse.json(
      { error: "An admin comment is required when requesting changes." },
      { status: 400 }
    );
  }
  ...
  if (status !== "pending_verification" && status !== "pending") {
    return NextResponse.json(
      { error: "Only listings pending verification can be returned for changes." },
      { status: 400 }
    );
  }
  ...
  await notifyListingReviewEvent({
    spaceId: id,
    eventType: "listing_needs_changes",
